...
class PokeRenderTask extends TimerTask {
	...
	String poketype;
	public void run() { // ripple = 2
		Log.d("size", "size: " + size + " addSize: " + addSize + " poketype: " + poketype + " mRenderer.runAnimation: " + mRenderer.runAnimation);
			...
			//if(getPackageName().toLowerCase().contains("full")) {
				...
				if(sliding.equals("2")) {
					if(this.mRenderer != null) {
						if(this.mRenderer.getViewportWidth() != 0) {
							float off = -(float)xPixelOffset/(float)this.mRenderer.getViewportWidth();
							System.out.println("off: " + off + " width: " + this.mRenderer.getViewportWidth() + " xOffset: " + xOffset + " yOffset: " + yOffset + " xOffsetStep: " + xOffsetStep + " yOffsetStep: " + yOffsetStep + " xPixelOffset: " + xPixelOffset + " yPixelOffset: " + yPixelOffset);
							this.mRenderer.setOffsetX(off);
						}
					}
				}
			//}
		...
		public void onTouchEvent (MotionEvent event) {
			...
			//if(getPackageName().toLowerCase().contains("full")) {
				...
				if(poketype.equals("2") || poketype.equals("3") || poketype.equals("4")) {
					...
					if(event.getAction() == event.ACTION_UP && !poketype.equals("2")) {
